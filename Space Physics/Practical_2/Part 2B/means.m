...
%
% Given three field components the function computes power and cross
% spectra. From the imaginary parts of the cross spectra the wave vector is
% obtained.
% Reference: Means, J. D., JGR, 77, 5551-5559, 1972
%
% INPUT 
...
%
% ntshift, ntsvar: time-averaging parameters, ntaver=number of fft-records
% to average over, ntshift=number of points to shift for each new record;
%
% NK, Kstart, Kshift: global time-parameters, NK=total number of time bins,
% Kstart=index where to start computation, Kshift=number of points to
% shift for each new time bin;
%
% OUTPUT
...
% deviations.
%
% IMPORTANT TO NOTE: On exit:
% If k dot B > 0 the wave is right-hand polarized, as for example, whistler mode waves
% The actually direction of propagation cannot be determined by this method but 
% is +k or -k.
% If k dot B < 0 the wave is instead left-handpolarized.


function [vpsd,vpsdz,vpsddev,vkx,vky,vkz,vkxs,vkys,vkzs,vttags,vfreqs]=means(bx, by, bz, ...
    FSAMP, NK, Kstart, Kshift, N, NG, GW, ntshift, ntaver)


%%% INITIALIZING %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if nargin < 12, ntshift=32; end
if nargin < 11, ntaver=5; end
...
%%% CREATING FREQUENCY AND TIME FILES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 tmid=0.5*((ntaver-1)*ntshift+NG);
 for l=1:NK
     vttags(l)=(Kstart+(l-1)*Kshift+tmid)/FSAMP;
 end
 
%%% GLOBAL TIME LOOP STARTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 

deltat=1./FSAMP;
for m=1:NK
    jstart=Kstart+(m-1)*Kshift;
    
%%% ZEROING LOCAL TIME LOOP PARAMETERS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
end % END: Local time loop


%%% Compute averages
nftaver=nfaver*ntaver;
...
%%% Compute PSDs    
    vpsd(m,j)=ftavgxp(j)+ftavgyp(j)+ftavgzp(j);
    vpsddev(m,j)=sqrt((ftavgxp(j)*ftavgxp(j))*(gxpdev(j)*gxpdev(j)) ...
        +(ftavgyp(j)*ftavgyp(j))*(gypdev(j)*gypdev(j)) ...
        +(ftavgzp(j)*ftavgzp(j))*(gzpdev(j)*gzpdev(j)))/vpsd(j);
    vpsdz(m,j)=ftavgzp(j);   
    vpsdx(m,j)=ftavgxp(j);   
    vabs=sqrt(ftavcyzi(j)*ftavcyzi(j)+ftavczxi(j)*ftavczxi(j) ...
        +ftavcxyi(j)*ftavcxyi(j));

%%% Compute Means-directions        
    vkx(m,j)=ftavcyzi(j)/vabs;
    vky(m,j)=ftavczxi(j)/vabs;
    vkz(m,j)=ftavcxyi(j)/vabs;
    vkxs(m,j)=cyzidev(j)/vabs;
    vkys(m,j)=czxidev(j)/vabs;
    vkzs(m,j)=cxyidev(j)/vabs;           
end % END: Spectra and Means-directions

end % END: Global time loop
